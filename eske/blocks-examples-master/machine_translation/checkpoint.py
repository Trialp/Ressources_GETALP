...
class SaveLoadUtils(object):
    """ Utility class for doing checkpoints. """
...
class LoadParameters(TrainingExtension, SaveLoadUtils):
    def __init__(self, saveto, **kwargs):
        self.folder = saveto
        super(LoadParameters, self).__init__(saveto, **kwargs)
...
class LoadNMT(LoadParameters):
    ...
    def load_iteration_state(self):
        # FIXME: iteration state is unaffected by current configuration,
        # so moving the training files can cause the iteration state to become invalid.
    ...
    def load_to(self, main_loop):
        ...
        super(LoadNMT, self).load_to(main_loop)
